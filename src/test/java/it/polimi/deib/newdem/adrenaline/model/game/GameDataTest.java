...
public class GameDataTest {
    ...
    @Test
    public void testGetPlayersData() throws Exception {
        try{
            uninitializedGameData.getPlayersData();
        }
        catch (IllegalStateException e) { /* ok */ }

        for(PlayerData p : gameData.getPlayersData()) {
            if(PlayerColor.MAGENTA == p.getColor()) return;
        }
    }
    ...
    public void testGetMapData() throws Exception {
        try{
            uninitializedGameData.getMapData();
        }
        catch (IllegalStateException e) { /* ok */ }

        assertEquals(gameData.getMapData().getRedSpawnPoint(),new TilePosition(0,0));
    }

    @Test
    public void testGetKillTrackData() throws Exception {
        try{
            uninitializedGameData.getKillTrackData();
        }
        catch (IllegalStateException e) { /* ok */ }

        assertEquals(gameData.getKillTrackData().getInitialLength(), gp.getKillTrackInitialLength());
    }

    @Test
    public void testGetPowerUpDeckData() throws Exception {
        try{
            uninitializedGameData.getPowerUpDeckData();
        }
        catch (IllegalStateException e) { /* ok */ }

        assertTrue(gameData.getPowerUpDeckData().getDiscardedCards().isEmpty());
        assertTrue(gameData.getPowerUpDeckData().getDrawableCards() > 0);
    }

    @Test
    public void testGetDropDeckData() throws Exception {
        try{
            uninitializedGameData.getDropDeckData();
        }
        catch (IllegalStateException e) { /* ok */ }

        assertTrue(gameData.getDropDeckData().getDiscardedCards().isEmpty());
        assertTrue(gameData.getDropDeckData().getDrawableCards() > 0);
    }

    @Test
    public void testGetWeaponDeckData() throws Exception {
        try{
            uninitializedGameData.getWeaponDeckData();
        }
        catch (IllegalStateException e) { /* ok */ }

        assertTrue(gameData.getWeaponDeckData().getDiscardedCards().isEmpty());
        assertTrue(gameData.getWeaponDeckData().getDrawableCards() > 0);
    }
